...
[float]
=== Deploy {kib} on Kubernetes

Use Elastic Cloud on Kubernetes (ECK) to deploy {kib} and {es} on Kubernetes.
For setup and configuration details, see
https://www.elastic.co/docs/deploy-manage/deploy/cloud-on-k8s[Elastic Cloud on Kubernetes documentation].
